...
const CreateFan = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      axios.get('https://sinftest7-k14w.onrender.com/api/fan/adminFan', { headers: { Authorization: `Bearer ${token}` } })
        .then((response) => setFanlar(response.data))
        .catch((error) => console.error('Fanlarni olishda xatolik:', error));

      axios.get('https://sinftest7-k14w.onrender.com/admin/admins', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://sinftest7-k14w.onrender.com/api/fan/create', fanData);
